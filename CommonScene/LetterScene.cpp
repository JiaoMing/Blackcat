...
LetterScene::LetterScene(){
    S_RM->addSceneRes("LetterScene", "letter");
}

LetterScene::~LetterScene(){
    S_RM->removeSceneRes("LetterScene");
}

CCScene* LetterScene::scene()
{
    CCScene* scene = CCScene::create();
    LetterScene *layer = LetterScene::create();
    scene->addChild(layer);
    return scene;
}

bool LetterScene::init()
{
    ...
    CCMenuItemToggle* toggle=CCMenuItemToggle::createWithTarget(this, menu_selector(LetterScene::menuCallBack), fk01Item1,fk01Item2,NULL);
    toggle->setPosition(ccp(1050, 230));
    toggle->setTag(TAG_TOGGLE);
    
    CCSprite* okSprite=CCSprite::createWithSpriteFrameName("OK.png");
    CCMenuItemSprite* ok=CCMenuItemSprite::create(okSprite, okSprite, this, menu_selector(LetterScene::menuCallBack));
    ...
    this->schedule(schedule_selector(LetterScene::scrollText), 0.05f);
...
void LetterScene::tableCellTouched(CCTableView* table, CCTableViewCell* cell)
{
    
}

CCSize LetterScene::cellSizeForTable(CCTableView *table)
{
    return CCSizeMake(860, 2400);
}

CCTableViewCell* LetterScene::tableCellAtIndex(CCTableView *table, unsigned int idx)
{
...
unsigned int LetterScene::numberOfCellsInTableView(CCTableView *table)
{
    return 1;
}


void LetterScene::scrollText(float t){
    CCPoint point=m_tableView->getContentOffset();
    point=ccp(point.x,point.y+2);
    if (point.y>=0) {
        m_tableView->setContentOffset(ccp(point.x, 0));
        this->unschedule(schedule_selector(LetterScene::scrollText));
    }else{
...
void LetterScene::menuCallBack(CCObject* obj){
...
#endif
void LetterScene::keyBackClicked(){
#if(CC_TARGET_PLATFORM == CC_PLATFORM_ANDROID)
	showTipDialogJNI("退出", "确定退出程序吗?");
#endif
}

void LetterScene::keyMenuClicked(){
